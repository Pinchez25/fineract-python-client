...
class LoanProductCreditAllocationRule(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["allocationTypes", "createdBy", "createdDate", "createdDateTime", "id", "lastModifiedBy", "lastModifiedDate", "lastModifiedDateTime", "loanProduct", "new", "transactionType"]
    ...
    def allocation_types_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['PENALTY', 'FEE', 'PRINCIPAL', 'INTEREST']):
                raise ValueError("each list item must be one of ('PENALTY', 'FEE', 'PRINCIPAL', 'INTEREST')")
        return value
    ...
    def transaction_type_validate_enum(cls, value):
        ...
        if value not in set(['CHARGEBACK']):
            raise ValueError("must be one of enum values ('CHARGEBACK')")
        return value
...
LoanProductCreditAllocationRule.model_rebuild(raise_errors=False)
